...
def push_back_column(a, A): 
    num_rows = len(A)
    if(len(a) != num_rows):
        print("The number of rows in the matrix is not equal to the number of elements in the array.")
    else:
        for i in range(0, num_rows):
            A[i].append(a[i])
...
plt.subplot(3, 1, 3)
plt.scatter(time_line, second_derivative_of_monthly_CPI, color = "steelblue", label = "second derivative of monthly CPI")
plt.legend(loc = "lower right")
plt.xticks(years)
plt.xlabel("Year")
